locals {
  thresholds = {
    ...
    ClusterTempStorageIopsEvaluationPeriods           = min(max(var.cluster_temp_storage_iops_evaluation_periods, 1), 100)
    ClusterTempStorageIopsThreshold                   = min(max(var.cluster_temp_storage_iops_threshold, 0), 100000000)
    ...
    InstanceReadIOPSEvaluationPeriods                 = min(max(var.instance_read_iops_evaluation_periods, 1), 100)
    InstanceReadIOPSThreshold                         = max(var.instance_read_iops_threshold, 0)
    ...
    InstanceWriteIOPSEvaluationPeriods                = min(max(var.instance_write_iops_evaluation_periods, 1), 100)
    InstanceWriteIOPSThreshold                        = max(var.instance_write_iops_threshold, 0)
